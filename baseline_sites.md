...
- generated through Wix and B12's native AI flows
    ...
    - input to Wix/B12 creates a home page, boilerplate, and blog posts
    - `agent/site_generation.py` uses a deterministic, checkpointed Playwright
      flow against a dedicated authenticated profile; it does not use a
      browser-control LLM
    - each site continues adding pages until it has 15 candidate pages, or is
      recorded as unclassifiable. The lightweight eligibility audit runs after
...
Freeze a random train/test split within each site category using
`agent/generation_dataset.py`. This is the paper-facing split: no paired or
causal-control design is required. Keep the split seed and test fraction with
the generated-site manifest.

## Historical training/test dataset 144x
